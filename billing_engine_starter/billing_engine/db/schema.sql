...
class CustomerRepository:
    ...
    def add(self, customer: Customer) -> Customer:
        if not isinstance(customer, Customer):
            raise TypeError("customer must be Customer")
        if customer.id is not None:
            raise ValueError("customer already has an id; use update?")
        ...
        # Return a new Customer with the id populated; other fields unchanged.
        return Customer(
            id=customer_id,
            name=customer.name,
            ...
            created_at=customer.created_at,  # may be None; DB default will fill
        )
...
class PlanRepository:
    ...
    def add(self, plan: Plan) -> Plan:
        ...
        # Convert pricing type and billing period to strings.
        pricing_type_str = plan.pricing_type.value
        billing_period_str = plan.billing_period.value
        # Store config_json as JSON string (if any)
        ...
        with self.db.connect() as conn:
            cur = conn.execute(
                ...
                VALUES (?, ?, ?, ?, ?)
                """,
                (plan.name, pricing_type_str, billing_period_str, plan.currency, config_json)
            )
            plan_id = cur.lastrowid
        # Return new Plan with id set.
        return Plan(
            id=plan_id,
            name=plan.name,
...
class SubscriptionRepository:
    ...
    def get_due_for_billing(self, as_of: date) -> list[Subscription]:
        # Active subscriptions with period_end <= as_of, plus trial subscriptions whose trial_end <= as_of.
        # We'll include both ACTIVE and TRIAL statuses.
        with self.db.connect() as conn:
            rows = conn.execute(
                """
                SELECT id, customer_id, plan_id, status,
...
class InvoiceRepository:
    ...
    def get(self, invoice_id: int) -> Optional[Invoice]:
        ...
        return Invoice(
            ...
            currency=row[4],
            subtotal=Money.from_storage(row[5], row[4]),
            discount_total=Money.from_storage(row[6], row[4]),
            tax_total=Money.from_storage(row[7], row[4]),
            total=Money.from_storage(row[8], row[4]),
...
class InvoiceLineItemRepository:
    ...
    def list_for_invoice(self, invoice_id: int) -> list[InvoiceLineItem]:
        ...
        # Need currency; we can get it from the invoice? We don't have it here, but we can assume
        # the calling code knows the currency. Since we don't have a currency field in line items,
        # we need to fetch it from the invoice. To avoid extra query, we can accept a currency parameter.
        # However, we can look up the invoice's currency from the invoice table.
        # But this method signature doesn't include currency. We'll fetch it internally.
        if not rows:
            return []
